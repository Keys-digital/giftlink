...
router.get('/', async (req, res, next) => {
    try {
        ...
        const gifts = await collection.find(query).toArray(); // Fetch filtered gifts

        res.json(gifts); // Return the filtered gifts as a JSON response
    } catch (e) {
        next(e); // Handle any errors during the search operation
    }
});
